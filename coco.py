...
class COCODetection(data.Dataset):
    ...
    def show_box(self, img, gts):
        ...
        for gt in gts:
            x1,y1,x2,y2 = gt[0]*w,gt[1]*h,gt[2]*w,gt[3]*h
            rect = Rectangle((x1,y1),x2-x1,y2-y1,color = 'red', alpha=0.5)
            plt.gca().add_patch(rect)
        plt.show()

    def show_mask(self, img, masks):
        import matplotlib.pyplot as plt
        plt.imshow(img)
        for mask in masks:
            plt.imshow(mask)
    ...
    def pull_item(self, index):
        ...
        # self.show_box(img, target)
        # self.show_mask(_img, masks)
